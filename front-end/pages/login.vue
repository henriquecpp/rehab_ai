<template>
  <div class="m-5 w-full max-w-md rounded-xl bg-white p-10 shadow-[0_20px_40px_rgba(0,0,0,0.1)]">
    <div class="mb-8 text-center">
      <div class="mb-4 flex items-center justify-center gap-3">
        <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br from-primary to-blue-500 text-2xl font-bold text-white">
          R
        </div>
        <span class="text-3xl font-bold text-gray-900">RehabAI</span>
      </div>
      <p class="text-base text-gray-600">Plataforma Inteligente de Prescrição Fisioterapêutica</p>
    </div>
    ...
    <form @submit.prevent="handleLogin">
      ...
      <div class="mb-5">
        ...
        <input v-model="password" type="password" class="form-input" placeholder="Sua senha" required>
      </div>
      <button type="submit" class="btn-primary mb-4 w-full justify-center py-2.5" :disabled="isLoading">
    ...
    <div class="mt-6 text-center">
      ...
      <button @click="isRegisterModalOpen = true" class="btn-secondary w-full justify-center py-2.5">
        Criar uma conta
      </button>
    </div>
  </div>
...
.btn-secondary { @apply flex items-center gap-2 rounded-lg bg-gray-200 px-5 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-300; }
</style>