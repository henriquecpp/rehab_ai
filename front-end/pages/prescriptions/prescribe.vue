<template>
  <UICard title="Nova Prescrição com IA">
    <div class="alert alert-info">
      <span>💡</span>
      <div>
        <strong>Fluxo Integrado:</strong> Upload do laudo → Extração OCR →
        Normalização → Geração IA → Plano estruturado
      </div>
    </div>
      ...
      >
        ...
        <div v-if="!selectedFile">
          ...
          <div style="font-weight: 600; margin-bottom: 8px">
            Clique ou arraste o laudo para cá
          </div>
...
const isValid = computed(
  () => selectedPatientId.value && selectedFile.value && !isLoading.value
);
...
const { data: patients, pending: patientsLoading } = await useApiFetch<
  PatientUser[]
>("/users", { query: { role: "PATIENT" } });
...
async function startPrescriptionWorkflow() {
  ...
  try {
    ...
    const fileResponse = await $fetch<{ id: string }>(
      `${config.public.apiBaseUrl}/files/upload`,
      {
        method: "POST",
        headers: {
...
.btn-primary {
  @apply flex items-center gap-2 rounded-lg bg-primary px-5 py-2 text-sm font-semibold text-white transition hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed;
}
.alert {
...
.alert-success {
  @apply border-success bg-green-50 text-green-800;
}
.upload-area {
  @apply cursor-pointer rounded-lg border-2 border-dashed border-gray-300 p-10 text-center transition;
}
.upload-area:hover {
  @apply border-primary bg-gray-50;
}
.upload-area.dragover {
  @apply border-primary bg-blue-50 ring-2 ring-primary;
}
</style>
