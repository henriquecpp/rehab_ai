<template>
  <div class="space-y-6">
    <div v-if="authStore.isPatient && redirecting" class="text-center py-8">
      <p class="text-gray-500">Carregando seus registros...</p>
    </div>

    <template v-else-if="authStore.isClinician">
      ...
      <!-- Seletor de Paciente (apenas para Clinician/Admin) -->
      ...
      <!-- Tabs de Categorias -->
      <div
        v-if="selectedPatientId || authStore.isPatient"
                ...
                <div class="flex justify-between items-start">
                  <div class="flex-1">
                    ...
                    <div class="mt-2 text-sm text-gray-700 space-y-1">
                      <p>
                        <span class="font-medium">Dose:</span> {{ med.dose }}
                      </p>
                      <p>
                        <span class="font-medium">Via:</span> {{ med.route }}
                      </p>
                      <p>
      ...
      <div v-else class="text-center py-12 bg-gray-50 rounded-lg">
        <p class="text-gray-500 text-lg">
          {{
            authStore.isClinician
              ? "Selecione um paciente para visualizar os registros médicos"
...
// If patient, redirect to their own history page
if (authStore.isPatient && authStore.user?.id) {
  redirecting.value = true;
  navigateTo(`/patients/${authStore.user.id}/history`);
}
...
async function submitNote() {
  const userId = selectedPatientId.value || authStore.user?.id;
  if (!userId) return;

  saving.value = true;
  try {
    await $api(`/patients/${userId}/history/notes`, {
      method: "POST",
...
async function submitCondition() {
  ...
  saving.value = true;
  try {
    await $api(`/patients/${userId}/conditions`, {
      method: "POST",
      body: {
...
async function submitAllergy() {
  ...
  saving.value = true;
  try {
    await $api(`/patients/${userId}/allergies`, {
      method: "POST",
      body: {
...
async function submitMedication() {
  ...
  saving.value = true;
  try {
    await $api(`/patients/${userId}/medications`, {
      method: "POST",
      body: {
...
async function submitVital() {
  ...
  saving.value = true;
  try {
    // Build valueJson based on type and value
    const valueJson = JSON.stringify({
      value: vitalForm.value.value,
      unit: vitalForm.value.unit,
    });

    await $api(`/patients/${userId}/vitals`, {
      method: "POST",
      body: {
...
async function submitProfile() {
  ...
  saving.value = true;
  try {
    await $api(`/patients/${userId}/profile`, {
      method: "PUT",
      body: {
...
async function fetchPatients() {
  if (!authStore.isClinician) return;

  try {
    const data = await $api<any[]>("/users?role=PATIENT&activeOnly=true");
    patients.value = data;
  } catch (error) {
...
async function fetchMedicalRecords() {
  ...
  try {
    ...
    const [
      ...
      $api(`/patients/${userId}/history/notes`),
      $api(`/patients/${userId}/conditions`),
      $api(`/patients/${userId}/allergies`),
      $api(`/patients/${userId}/medications`),
      $api(`/patients/${userId}/vitals`),
    ]);
    ...
    // Pre-populate profile form if profile exists
    if (profile.value) {
      profileForm.value = {
...
async function deleteNote(id: string) {
  ...
  const userId = selectedPatientId.value || authStore.user?.id;
  try {
    await $api(`/patients/${userId}/history/notes/${id}`, { method: "DELETE" });
...
async function deleteCondition(id: string) {
  ...
  const userId = selectedPatientId.value || authStore.user?.id;
  try {
    await $api(`/patients/${userId}/conditions/${id}`, { method: "DELETE" });
...
async function deleteAllergy(id: string) {
  ...
  const userId = selectedPatientId.value || authStore.user?.id;
  try {
    await $api(`/patients/${userId}/allergies/${id}`, { method: "DELETE" });
...
async function deleteMedication(id: string) {
  ...
  const userId = selectedPatientId.value || authStore.user?.id;
  try {
    await $api(`/patients/${userId}/medications/${id}`, { method: "DELETE" });
